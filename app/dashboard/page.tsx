...
export default async function Dashboard({ searchParams }: DashboardProps) {
  ...
  const catalog = ((products ?? []) as Product[]).map((product) => {
    const productPackages = ((packages ?? []) as Package[])
      .filter((pkg) => pkg.product_id === product.id)
      .sort((a, b) => a.sort_order - b.sort_order);

    return {
      ...product,
      packages: productPackages.map((pkg) => {
  ...
  return (
    <main className="dashboardPage">
      {params.success && (
        <div className="loginAlert loginAlertSuccess" role="alert">
          <span>✓</span>
          <div><strong>Sign in successful</strong><p>{params.success}</p></div>
        </div>
      )}
      ...
      <div className="dashboardWrap">
        ...
        <section className="dashboardHero">
          <div>
            <span className="miniLabel">JBE BUSINESS PORTAL</span>
            <h1>Welcome back{profile.full_name ? `, ${profile.full_name}` : ''}.</h1>
            <p>Manage your reseller orders with JBE pricing and business tools.</p>
          </div>
          <div className="dashboardStats">
            <div><b>{catalog.length}</b><span>Products</span></div>
            <div><b>{totalPackages}</b><span>Packages</span></div>
            <div><b>B2B</b><span>Reseller pricing</span></div>
          </div>
        </section>

        <section className="dashboardSectionHead">
          <div><span className="miniLabel">RESELLER CATALOG</span><h2>Available packages</h2></div>
          <span className="publicBadge">RESELLER PRICES</span>
        </section>

        <section className="dashboardGrid">
          {catalog.map((product) => (
            <article className="dashboardProductCard" key={product.id}>
              <div className="dashboardProductHead">
                <span className="productIcon"><span>{product.icon || 'JBE'}</span><i /></span>
                <div>
                  <span className="cardKicker">JBE BUSINESS</span>
                  <h3>{product.name}</h3>
                </div>
                <span className="countPill">{product.packages.length} plans</span>
              </div>

              <div className="dashboardPackageList">
                {product.packages.map((pkg) => (
                  <div className="dashboardPackageRow" key={pkg.id}>
                    <div><strong>{pkg.name}</strong><span>Reseller price</span></div>
                    <b>{formatKs(pkg.price)}</b>
                  </div>
                ))}
              </div>
            </article>
        ...
        <footer className="dashboardFooter">
          ...
          <span>Private reseller portal • {profile.role}</span>
        </footer>
      </div>
    </main>
  );

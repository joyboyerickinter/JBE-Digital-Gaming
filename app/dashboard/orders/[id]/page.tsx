...
export default async function OrderDetail({params}:{params:Promise<{id:string}>}) {
  ...
  const {data:order}=await supabase.from('orders').select('id,order_number,customer_identifier,customer_identifier_type,customer_name,payment_method,transaction_last6,status,cancel_reason,total_amount,created_at,order_items(product_name,package_name,reseller_price,quantity,line_total,sort_order)').eq('id',(await params).id).eq('reseller_id',userId).maybeSingle();
  if(!order) notFound();

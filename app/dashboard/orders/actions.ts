...
export async function createGuestOrder(formData: FormData) {
  ...
  const cleanItems = items.map(item => {
    const pkg: any = packageMap.get(item.package_id);
    const price = Number(pkg?.prices?.find((p: any) => p.customer_type === 'b2c' && p.active)?.price ?? 0);
    return {
      ...item,
      product_name: String(pkg?.products?.name || ''),
      package_name: String(pkg?.name || ''),
      product_active: Boolean(pkg?.products?.active),
      b2c_price: price,
    };
  });
  ...
  if (cleanItems.some(item =>
    ...
    item.b2c_price <= 0 ||
    !item.product_active ||
    item.product_id !== String(packageMap.get(item.package_id)?.product_id || '')
  )) {
    redirect(errorPath + '?error=One%20or%20more%20packages%20are%20unavailable');
  }

  const total = cleanItems.reduce((sum, item) => sum + item.b2c_price * item.quantity, 0);
  if (total <= 0) redirect(errorPath + '?error=Invalid%20order%20total');
  ...
  if (!['image/jpeg', 'image/png', 'image/webp'].includes(screenshot.type)) {
    redirect(errorPath + '?error=Use%20JPG%2C%20PNG%20or%20WEBP%20screenshot');
  }
  ...
  const extension = screenshot.type === 'image/png' ? 'png' : screenshot.type === 'image/webp' ? 'webp' : 'jpg';
  const screenshotPath = 'guest/' + String(order.id) + '.' + extension;
  ...
  try {
    await sendAdminNewOrderEmail({
      order_number: String(order.order_number),
      reseller_name: 'Website customer',
      customer_name: customerName,
      customer_identifier: identifier,
      customer_identifier_type: identifierType,
      payment_method: paymentMethod,

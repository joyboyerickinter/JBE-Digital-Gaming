...
function ProductCard({ product }: { product: CatalogProduct }) {
  const isOutline = product.slug.includes('outline');
...
export default async function Home() {
  ...
  const [catalog, authResult] = await Promise.all([getCatalog(), supabase.auth.getClaims()]);
  ...
  if (userId) {
    ...
    if (profile?.active) { destination = profile.role === 'admin' ? '/admin' : '/dashboard'; actionLabel = profile.role === 'admin' ? 'Admin Portal →' : 'Go to Dashboard →'; }
  }
